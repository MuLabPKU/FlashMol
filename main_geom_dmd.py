...
parser.add_argument('--gan_pos', type=int, default=0)
...
# Create teacher, G, mu_fake, discriminator
teacher = None
...
if args.train_diffusion:
    if args.teacher_path is not None:
        ...
        if args.teacher_epoch is not None:
            ...
            ema_model_file     = join(args.teacher_path, f'generative_model_ema_{args.teacher_epoch}.npy')
            vanilla_model_file = join(args.teacher_path, f'generative_model_{args.teacher_epoch}.npy')
            print(f"Loading teacher from epoch {args.teacher_epoch}")
        else:
            args_file = join(args.teacher_path, 'args.pickle')
            ema_model_file     = join(args.teacher_path, 'generative_model_ema.npy')
            vanilla_model_file = join(args.teacher_path, 'generative_model.npy')
        ...
        # Load the checkpoint — EMA first, vanilla fallback
        import os
        if os.path.exists(ema_model_file):
            model_file = ema_model_file
        elif os.path.exists(vanilla_model_file):
            model_file = vanilla_model_file
            print(f"WARNING: EMA checkpoint not found, falling back to vanilla model")
        else:
            raise FileNotFoundError(
                f"Teacher model file not found (tried EMA and vanilla):\n"
                f"  {ema_model_file}\n  {vanilla_model_file}\n"
                f"Make sure the teacher checkpoint exists at the specified epoch."
            )
        ...
        raise ValueError(
            ...
            "If you want to train from scratch, use main_geom_drugs.py instead."
        )
else:
...
def main():
    ...
    for epoch in range(args.start_epoch, args.n_epochs):
        ...
        train_epoch(args=args, loader=dataloaders['train'], epoch=epoch,
                    ...
                    optim_G=optim_G, optim_fake=optim_fake, optim_d=optim_d,
                    prop_dist=prop_dist,
        ...
        if epoch % args.test_epochs == 0:
            ...
            nll_val = test(args=args, loader=dataloaders['val'], epoch=epoch,
