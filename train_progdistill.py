...
def train_epoch(args, loader, epoch, teacher, model, model_dp, model_ema, ema, device, dtype, property_norms, optim,
    ...
    for i, data in enumerate(loader):
        ...
        # Skip sampling in early epochs (0-8) when student model is too unstable
        if (epoch % args.test_epochs == 0) and (i % args.visualize_every_batch == 0) and not (epoch == 0 and i == 0) and epoch >= 8 and args.train_diffusion:
            start = time.time()
            if len(args.conditioning) > 0:
